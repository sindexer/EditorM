...
$baselineZip = Join-Path $parent "visual_authoring_engine_phase0e_r2_review_2026-08-11.zip"
$expectedBaselineHash = "2292fb079d4a5b6da4bedcd3f026277a6b2c6ae372193616112e1b2766ff2bb1"
$zipName = "visual_authoring_engine_phase0e_r3_review_2026-08-11.zip"
$zipPath = Join-Path $parent $zipName
$sidecarPath = "$zipPath.sha256"
$scratch = Join-Path $env:TEMP ("phase0e-r3-package-" + [Guid]::NewGuid().ToString("N"))
...
function Get-ExclusionCategory([string]$RelativePath) {
    ...
    if ($path -match "^tools/.*codemod.*\.mjs$" -or $path -match "^tools/phase0e_.*\.mjs$") { return "work_only_codemods" }
    if ($path -in @(
        ...
        "docs/verification/PHASE_0E_R3_BROWSER_FAILURE.json"
...
if (Test-Path -LiteralPath $zipPath) { throw "Refusing to overwrite existing R3 ZIP: $zipPath" }
if (Test-Path -LiteralPath $sidecarPath) { throw "Refusing to overwrite existing R3 sidecar: $sidecarPath" }
...
try {
    ...
    if ($sourceAdded -notcontains "docs/PHASE_0E_R3_CHANGE_MANIFEST.json") {
        $sourceAdded = @($sourceAdded + "docs/PHASE_0E_R3_CHANGE_MANIFEST.json" | Sort-Object)
    }
    ...
    $manifestPath = Join-Path $repoRoot "docs/PHASE_0E_R3_CHANGE_MANIFEST.json"
    ...
    try {
        ...
        $forbidden = @($names | Where-Object {
            ...
            $_ -match "^WebEditor/tools/.*codemod.*\.mjs$" -or $_ -match "^WebEditor/tools/phase0e_.*\.mjs$" -or
