param(
    [string]$VerificationPath = "docs/verification/PHASE_0E_VERIFICATION.txt"
)
...
$scratch = Join-Path $env:TEMP ("phase0e-verify-" + [Guid]::NewGuid().ToString("N"))
$started = Get-Date
...
try {
    try {
        ...
        Invoke-VerificationStep "Phase 0E generated WASM package" "powershell -NoProfile -ExecutionPolicy Bypass -File tools/build-phase0e-wasm.ps1" {
        ...
        try {
            Invoke-VerificationStep "Phase 0E web clean install" "npm ci --ignore-scripts" {
                & $npm ci --ignore-scripts
            }
            Invoke-VerificationStep "Phase 0E React production build" "npm run build" {
                & $npm run build
            }
            Invoke-VerificationStep "Phase 0E web unit tests" "npm test" {
                & $npm test
            }
        } finally {
            Pop-Location
        ...
        Invoke-VerificationStep "Phase 0E native structural proof" "cargo run --release -p visual_authoring_runtime --bin phase0e_proof -- docs/PHASE_0E_METRICS.json" {
            & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "cargo.ps1") run --release -p visual_authoring_runtime --bin phase0e_proof -- docs/PHASE_0E_METRICS.json
        }
        Assert-ProofPassed (Join-Path $repoRoot "docs/PHASE_0E_METRICS.json") "phase0e_native"

        Push-Location $editorRoot
        try {
            Invoke-VerificationStep "Phase 0E self-contained actual hardware browser proof" "npm run test:browser" {
                & $npm run test:browser
            }
        } finally {
            Pop-Location
        }
        Assert-ProofPassed (Join-Path $repoRoot "docs/verification/PHASE_0E_BROWSER_PROOF.json") "phase0e_browser"
        Assert-ProofPassed (Join-Path $repoRoot "docs/verification/PHASE_0E_PIXEL_READBACK.json") "phase0e_pixel"
    ...
    $finished = Get-Date
    $header = @(
        "Phase 0E verification raw output"
