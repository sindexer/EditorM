...
try {
    ...
    if (-not $health.ok) { throw "phase0e_preview_not_ready: health endpoint did not become ready" }
    ...
    Write-Output "Phase 0E editor: $url"
    Write-Output "runtime=Dedicated Worker WASM core + main-thread actual WebGPU renderer"
    Write-Output "health=PASS assets=PASS mime=PASS"
