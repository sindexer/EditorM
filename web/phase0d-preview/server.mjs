...
const port = Number(portArgument?.split("=")[1] ?? process.env.PHASE0D_PORT ?? 4173);
...
const server = createServer(async (request, response) => {
  try {
    if (request.url === "/__health") {
      ...
      response.end(JSON.stringify({ ok: true, phase: "0D" }));
      return;
    }
...
server.listen(port, "127.0.0.1", () => {
  console.log(`Phase 0D preview: http://127.0.0.1:${port}/`);
});