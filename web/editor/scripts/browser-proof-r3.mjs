...
const proofPath = path.join(verificationRoot, "PHASE_0E_R3_BROWSER_PROOF.json");
const pixelPath = path.join(
  verificationRoot,
  "PHASE_0E_R3_PIXEL_READBACK.json",
);
const failurePath = path.join(
  verificationRoot,
  "PHASE_0E_R3_BROWSER_FAILURE.json",
);
const screenshots = {
  default: path.join(verificationRoot, "phase0e-r3-editor-default.png"),
  selection: path.join(verificationRoot, "phase0e-r3-editor-selection.png"),
  nested_group: path.join(
    verificationRoot,
    "phase0e-r3-editor-nested-group.png",
  ),
  component_showcase: path.join(
    verificationRoot,
    "phase0e-r3-component-showcase.png",
  ),
  hundred_k_debug: path.join(verificationRoot, "phase0e-r3-100k-structure.png"),
};
const chrome =
...
try {
  ...
  await evaluate(
    `window.__phase0eSend("command", ${JSON.stringify({ command: { kind: "group", group_id: tenKGroupId, name: "R2 10k Group", targets: tenKTargets } })})`,
  );
  ...
  await evaluate(
    `window.__phase0eSend("command", ${JSON.stringify({ command: { kind: "group", group_id: hundredKGroupId, name: "R2 100k Group", targets: hundredKTargets } })})`,
  );
  ...
  async function runR2StructuralBenchmark(fixture, nodeCount, timeoutMs) {
    await evaluate(`window.__phase0eSend("load_fixture", { fixture: ${JSON.stringify(fixture)} }).then(() => true)`);
    await waitFor(
      `window.__PHASE0E_PROOF__?.fixture === '${fixture === "bench-b" ? "BENCH-B" : "BENCH-C"}' && window.__PHASE0E_PROOF__?.projection_nodes === ${nodeCount + 1}`,
      timeoutMs,
      `r2_${nodeCount}_load_timeout`,
    );
    const targets = [1, Math.floor(nodeCount / 2) + 1, nodeCount].map(fixtureNodeId);
    const groupId = fixtureNodeId(BigInt(nodeCount) + 5000000n);
    const workflowGroupId = fixtureNodeId(BigInt(nodeCount) + 8000000n);
    const insertedId = fixtureNodeId(BigInt(nodeCount) + 8000001n);
    const rootId = fixtureNodeId(0);
    const deletedId = fixtureNodeId(2);
    const reorderedId = fixtureNodeId(nodeCount - 1);
    return evaluate(`(async () => {
      const send = window.__phase0eSend;
      const ui = window.__phase0eR2Counters;
      if (!send || !ui) throw new Error("R2 browser diagnostics are unavailable");
      const targets = ${JSON.stringify(targets)};
      const groupId = ${JSON.stringify(groupId)};
      const workflowGroupId = ${JSON.stringify(workflowGroupId)};
      const insertedId = ${JSON.stringify(insertedId)};
      const rootId = ${JSON.stringify(rootId)};
      const deletedId = ${JSON.stringify(deletedId)};
      const reorderedId = ${JSON.stringify(reorderedId)};
      const nodeCount = ${nodeCount};
      const delta = (after, before) => Object.fromEntries(Object.keys(after).map((key) => [key, after[key] - (before[key] ?? 0)]));
      const engineWork = (response) => ["document", "scene"].reduce((sum, prefix) => {
        ...
        return sum + Math.max(metric("entries_examined"), metric("rank_order_comparisons")) + metric("entries_copied") + metric("entries_moved_or_shifted") + metric("nodes_allocated") + Math.ceil(metric("allocated_bytes") / 8) + metric("tree_rebalances") + metric("fallback_or_rebuild_count") + metric("full_scans") + metric("full_copies") + metric("dense_index_rewrites");
      }, 0);
      const uiWork = (work) => Math.max(work.sequenceEntriesExamined, work.sequenceRankOrderComparisons) + work.sequenceEntriesCopied + work.sequenceEntriesMovedOrShifted +
        work.sequenceNodesAllocated + Math.ceil(work.sequenceAllocatedBytes / 8) + work.sequenceTreeRebalances + work.sequenceFallbackOrRebuildCount + work.sequenceFullScans + work.sequenceFullCopies + work.sequenceDenseIndexRewrites;
      const assertBounded = (response, work, requireGpuZero = true) => {
        const failures = [];
        for (const prefix of ["document", "scene"]) for (const suffix of ["full_scans", "full_copies", "dense_index_rewrites"]) {
          const key = prefix + "_sequence_" + suffix;
          if (Number(response.metrics[key] ?? 0) !== 0) failures.push(key);
        }
        for (const key of ["sequenceFullScans", "sequenceFullCopies", "sequenceDenseIndexRewrites", "hierarchyFullRebuilds"]) {
          if (Number(work[key] ?? 0) !== 0) failures.push("ui." + key);
        }
        ...
        if (Number(response.metrics.ui_full_snapshots ?? 0) !== 0) failures.push("ui_full_snapshots");
        if (requireGpuZero && (Number(response.render_delta.dirty_slots ?? 0) !== 0 || Number(response.metrics.instance_upload_bytes ?? 0) !== 0)) failures.push("gpu_structural_delta");
        if (failures.length) throw new Error("R2 bounded assertion failed: " + failures.join(","));
      };
      const operations = Object.fromEntries(["group", "undo", "redo", "ungroup", "edited_ungroup"].map((name) => [name, { times_ms: [], work: [], payload_bytes: [] }]));
      for (let iteration = 0; iteration < 40; iteration += 1) {
        let before = ui();
        let started = performance.now();
        const grouped = await send("command", { command: { kind: "group", group_id: groupId, name: "R2 browser group", targets } });
        let elapsed = performance.now() - started;
        let uiDelta = delta(ui(), before);
        assertBounded(grouped, uiDelta);
        if (iteration >= 10) { operations.group.times_ms.push(elapsed); operations.group.work.push(engineWork(grouped) + uiWork(uiDelta)); operations.group.payload_bytes.push(grouped.metrics.ui_projection_payload_bytes); }

        before = ui(); started = performance.now();
        const undone = await send("undo"); elapsed = performance.now() - started; uiDelta = delta(ui(), before);
        assertBounded(undone, uiDelta);
        if (iteration >= 10) { operations.undo.times_ms.push(elapsed); operations.undo.work.push(engineWork(undone) + uiWork(uiDelta)); operations.undo.payload_bytes.push(undone.metrics.ui_projection_payload_bytes); }

        before = ui(); started = performance.now();
        const redone = await send("redo"); elapsed = performance.now() - started; uiDelta = delta(ui(), before);
        assertBounded(redone, uiDelta);
        if (iteration >= 10) { operations.redo.times_ms.push(elapsed); operations.redo.work.push(engineWork(redone) + uiWork(uiDelta)); operations.redo.payload_bytes.push(redone.metrics.ui_projection_payload_bytes); }

        before = ui(); started = performance.now();
        const ungrouped = await send("command", { command: { kind: "ungroup", node_id: groupId } }); elapsed = performance.now() - started; uiDelta = delta(ui(), before);
        assertBounded(ungrouped, uiDelta);
        if (iteration >= 10) { operations.ungroup.times_ms.push(elapsed); operations.ungroup.work.push(engineWork(ungrouped) + uiWork(uiDelta)); operations.ungroup.payload_bytes.push(ungrouped.metrics.ui_projection_payload_bytes); }
      }

      for (let iteration = 0; iteration < 40; iteration += 1) {
        const steps = [
          await send("command", { command: { kind: "group", group_id: workflowGroupId, name: "R2 edited group", targets } }),
          await send("command", { command: { kind: "create_shape", node_id: insertedId, parent_id: rootId, index: 0, shape: "rectangle", name: "Inserted", x: 0, y: 0, width: 1, height: 1 } }),
          await send("command", { command: { kind: "delete_node", node_id: deletedId } }),
          await send("command", { command: { kind: "reparent", node_id: reorderedId, parent_id: rootId, index: 1, preserve_world: false } }),
        ];
        for (let step = 0; step < steps.length; step += 1) assertBounded(steps[step], {}, false);
        const before = ui();
        const started = performance.now();
        const finalUngroup = await send("command", { command: { kind: "ungroup", node_id: workflowGroupId } });
        const elapsed = performance.now() - started;
        const uiDelta = delta(ui(), before);
        assertBounded(finalUngroup, uiDelta, true);
        if (iteration >= 10) {
          operations.edited_ungroup.times_ms.push(elapsed);
          operations.edited_ungroup.work.push(engineWork(finalUngroup) + uiWork(uiDelta));
          operations.edited_ungroup.payload_bytes.push(Number(finalUngroup.metrics.ui_projection_payload_bytes ?? 0));
        }
        for (let reset = 0; reset < 5; reset += 1) await send("undo");
      }

      await send("command", { command: { kind: "group", group_id: groupId, name: "R2 persisted group", targets } });
      const savedStarted = performance.now();
      const saved = await send("save_document");
      const saveMs = performance.now() - savedStarted;
      const documentJson = saved.result.document_json;
      const loadedStarted = performance.now();
      const loaded = await send("load_document", { document_json: documentJson });
      const loadMs = performance.now() - loadedStarted;
      const beforeLoadedUngroup = ui();
      const loadedUngroup = await send("command", { command: { kind: "ungroup", node_id: groupId } });
      const loadedUngroupUi = delta(ui(), beforeLoadedUngroup);
      assertBounded(loadedUngroup, loadedUngroupUi);

      const percentile = (values, fraction) => [...values].sort((a, b) => a - b)[Math.ceil((values.length - 1) * fraction)];
      const summarized = Object.fromEntries(Object.entries(operations).map(([name, value]) => [name, {
        samples_ms: value.times_ms,
        median_ms: percentile(value.times_ms, 0.50),
        p95_ms: percentile(value.times_ms, 0.95),
        work_samples: value.work,
        max_work: Math.max(...value.work),
        max_projection_payload_bytes: Math.max(...value.payload_bytes),
      }]));
      return {
        ...
        persistence: { save_ms: saveMs, load_ms: loadMs, document_bytes: saved.result.document_bytes, loaded: loaded.result.loaded, loaded_ungroup_bounded: true },
        final_ui: ui(),
      };
    })()`);
  }

  const r2TenK = await runR2StructuralBenchmark("bench-b", 10_000, 120000);
  const r2HundredK = await runR2StructuralBenchmark("bench-c", 100_000, 240000);
  const r2OperationNames = ["group", "undo", "redo", "ungroup", "edited_ungroup"];
  const r2StructuralRatios = Object.fromEntries(r2OperationNames.map((name) => [name, {
    work_count: r2HundredK.operations[name].max_work / Math.max(1, r2TenK.operations[name].max_work),
    median_time: r2HundredK.operations[name].median_ms / Math.max(Number.EPSILON, r2TenK.operations[name].median_ms),
    projection_payload: r2HundredK.operations[name].max_projection_payload_bytes / Math.max(1, r2TenK.operations[name].max_projection_payload_bytes),
  }]));
  const r2StructuralPassed = Object.values(r2StructuralRatios).every((ratio) => ratio.work_count <= 1.35 && ratio.median_time <= 3.0 && ratio.projection_payload <= 1.35) &&
    r2TenK.final_ui.mountedRows <= 30 && r2HundredK.final_ui.mountedRows <= 30;

  const r3MatrixSource = await readFile(path.join(scriptRoot, "r3-browser-matrix-page.js"), "utf8");
  await evaluate(r3MatrixSource);
  const r3ComplexityMatrix = await evaluate("globalThis.__runPhase0eR3Matrix()");
  ...
  const checks = {
    r3_complexity_matrix: r3ComplexityMatrix.all_passed === true,
    r2_structural_10k_100k_bounded: r2StructuralPassed,
  ...
  const proof = {
    ...
    runtime: final,
    r3_complexity_matrix: r3ComplexityMatrix,
    r2_structural_benchmarks: { ten_k: r2TenK, hundred_k: r2HundredK, ratios_100k_over_10k: r2StructuralRatios },
    scenarios: {
      initial,
      layers: {
  ...
  console.log(
    `Phase 0E-R3 actual browser proof: ${proof.all_passed ? "PASS" : "FAIL"}`,
  );
