...
const proofPath = path.join(verificationRoot, "PHASE_0E_R2_BROWSER_PROOF.json");
const pixelPath = path.join(
  verificationRoot,
  "PHASE_0E_R2_PIXEL_READBACK.json",
);
const failurePath = path.join(
  verificationRoot,
  "PHASE_0E_R2_BROWSER_FAILURE.json",
);
const screenshots = {
  default: path.join(verificationRoot, "phase0e-r2-editor-default.png"),
  selection: path.join(verificationRoot, "phase0e-r2-editor-selection.png"),
  nested_group: path.join(
    verificationRoot,
    "phase0e-r2-editor-nested-group.png",
  ),
  component_showcase: path.join(
    verificationRoot,
    "phase0e-r2-component-showcase.png",
  ),
  hundred_k_debug: path.join(verificationRoot, "phase0e-r2-100k-structure.png"),
};
const chrome =
...
try {
  ...
  async function runR2StructuralBenchmark(fixture, nodeCount, timeoutMs) {
    ...
    return evaluate(`(async () => {
      ...
      const engineWork = (response) => ["document", "scene"].reduce((sum, prefix) => {
        ...
        return sum + Math.max(metric("entries_examined"), metric("rank_order_comparisons")) + metric("entries_copied") + metric("entries_moved_or_shifted") + metric("nodes_allocated") + metric("full_scans") + metric("full_copies") + metric("dense_index_rewrites");
      }, 0);
      const uiWork = (work) => Math.max(work.sequenceEntriesExamined, work.sequenceRankOrderComparisons) + work.sequenceEntriesCopied + work.sequenceEntriesMovedOrShifted +
        work.sequenceNodesAllocated + work.sequenceFullScans + work.sequenceFullCopies + work.sequenceDenseIndexRewrites;
  ...
  console.log(
    `Phase 0E-R2 actual browser proof: ${proof.all_passed ? "PASS" : "FAIL"}`,
  );
