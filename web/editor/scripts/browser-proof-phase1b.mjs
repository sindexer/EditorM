...
const proofPath = path.join(verificationRoot, "PHASE_1B_BROWSER_PROOF.json");
...
async function runPhase1ALegacy() {
try {
...
const phase1bScreenshots = {
  workspace: path.join(verificationRoot, "phase1b-multislide-workspace.png"),
  timeline: path.join(verificationRoot, "phase1b-layers-timeline.png"),
};
...
try {
  ...
  const gpuInfo = await browserClient.send("SystemInfo.getInfo");
  const target = await (
    await fetch(debugBase + "/json/new?" + encodeURIComponent(preview.url), { method: "PUT" })
  ).json();
  pageClient = new CdpClient(target.webSocketDebuggerUrl);
  await pageClient.connect();
  pageClient.on("Runtime.exceptionThrown", ({ exceptionDetails }) => {
  ...
  await waitForReady();
  await waitFor("window.__PHASE0E_PROOF__?.editor_session?.slides?.length === 1");
  await waitFor("window.__PHASE0E_PROOF__?.thumbnails?.cached === 1 && window.__PHASE0E_PROOF__?.thumbnails?.errors === 0", 45000, "initial_thumbnail_timeout");
  const initial = await getProof();
  const firstSlide = initial.editor_session.active_slide_id;
  const firstShape = "30000000-0000-0000-0000-000000000001";
  const secondShape = "30000000-0000-0000-0000-000000000002";
  const duplicateSlide = "30000000-0000-0000-0000-000000000003";

  const firstCreated = await send("command", { command: {
    kind: "create_shape", node_id: firstShape, parent_id: firstSlide, index: 0,
    shape: "rectangle", name: "First slide object", x: 120, y: 120, width: 160, height: 80,
  }});
  await send("selection", { target: firstShape, mode: "replace" });
  const firstState = await send("camera", { camera: { kind: "pan", dx: 36, dy: -24 } });

  await clickSelector('[data-testid="add-slide"]');
  await waitFor("window.__PHASE0E_PROOF__?.editor_session?.slides?.length === 2");
  const afterAdd = await getProof();
  const secondSlide = afterAdd.editor_session.active_slide_id;
  const secondCreated = await send("command", { command: {
    kind: "create_shape", node_id: secondShape, parent_id: secondSlide, index: 0,
    shape: "ellipse", name: "Second slide object", x: 120, y: 120, width: 160, height: 80,
  }});
  await send("selection", { target: secondShape, mode: "replace" });
  const secondState = await send("camera", { camera: { kind: "pan", dx: -48, dy: 30 } });
  await waitFor("window.__PHASE0E_PROOF__?.thumbnails?.cached === 2 && window.__PHASE0E_PROOF__?.thumbnails?.errors === 0", 45000, "two_thumbnail_timeout");
  await capture(phase1bScreenshots.workspace);

  const restoredFirst = await send("slide", { slide: { kind: "activate", slide_id: firstSlide } });
  const firstWorld = transformPoint(firstCreated.projection.upserts.find((node) => node.id === firstShape).world_transform, [80, 40]);
  const firstPoint = [
    (firstWorld[0] - restoredFirst.camera.center[0]) * restoredFirst.camera.zoom + restoredFirst.camera.viewport[0] / 2,
    (firstWorld[1] - restoredFirst.camera.center[1]) * restoredFirst.camera.zoom + restoredFirst.camera.viewport[1] / 2,
  ];
  const firstHit = await send("hit_test", { x: firstPoint[0], y: firstPoint[1] });
  const outsideSelection = await evaluate(`window.__phase0eSend("selection", ${JSON.stringify({ target: secondShape, mode: "replace" })}).then(() => null).catch((error) => ({ code: error.code, message: error.message }))`);
  const restoredSecond = await send("slide", { slide: { kind: "activate", slide_id: secondSlide } });
  const secondWorld = transformPoint(secondCreated.projection.upserts.find((node) => node.id === secondShape).world_transform, [80, 40]);
  const secondPoint = [
    (secondWorld[0] - restoredSecond.camera.center[0]) * restoredSecond.camera.zoom + restoredSecond.camera.viewport[0] / 2,
    (secondWorld[1] - restoredSecond.camera.center[1]) * restoredSecond.camera.zoom + restoredSecond.camera.viewport[1] / 2,
  ];
  const secondHit = await send("hit_test", { x: secondPoint[0], y: secondPoint[1] });

  const duplicated = await send("slide", { slide: {
    kind: "duplicate", source_id: secondSlide, slide_id: duplicateSlide, index: 2,
  }});
  const duplicateChildren = duplicated.projection.upserts
    .filter((node) => node.parent_id === duplicateSlide)
    .map((node) => node.id);
  await send("slide", { slide: { kind: "rename", slide_id: duplicateSlide, name: "Opening copy" } });
  const reordered = await send("slide", { slide: { kind: "reorder", slide_id: duplicateSlide, index: 0 } });
  const saved = await send("save_document");
  const deleted = await send("slide", { slide: { kind: "delete", slide_id: duplicateSlide } });
  const undone = await send("undo");
  const redone = await send("redo");
  const restored = await send("load_document", { document_json: saved.result.document_json });

  await send("slide", { slide: { kind: "activate", slide_id: secondSlide } });
  const beforeEdit = await getProof();
  await send("command", { command: {
    kind: "set_transform", node_id: secondShape, matrix: [1, 0, 0, 1, 160, 145],
  }});
  await waitFor(`window.__PHASE0E_PROOF__?.thumbnails?.invalidations > ${beforeEdit.thumbnails.invalidations}`);
  await waitFor("window.__PHASE0E_PROOF__?.thumbnails?.cached === 3 && window.__PHASE0E_PROOF__?.thumbnails?.pending === 0 && window.__PHASE0E_PROOF__?.thumbnails?.errors === 0", 45000, "thumbnail_queue_drain_timeout");
  const afterEdit = await getProof();

  const panelState = await evaluate(`(async () => {
    const before = JSON.parse(localStorage.getItem("editorm.phase1b.workspace"));
    const splitters = [...document.querySelectorAll('[role="separator"]')];
    splitters[0]?.dispatchEvent(new KeyboardEvent("keydown", { key: "ArrowRight", bubbles: true }));
    await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));
    const after = JSON.parse(localStorage.getItem("editorm.phase1b.workspace"));
    const layerIds = [...document.querySelectorAll('.layers-viewport [data-node-id]')].map((entry) => entry.dataset.nodeId);
    const timelineIds = [...document.querySelectorAll('.timeline-viewport [data-node-id]')].map((entry) => entry.dataset.nodeId);
    return { splitter_count: splitters.length, before, after, layer_ids: layerIds, timeline_ids: timelineIds };
  })()`);
  await clickSelector('[data-testid="tool-ellipse"]');
  await send("selection", { target: null, mode: "clear" });
  await waitFor("document.querySelector('[data-inspector-context=\"tool\"]')");
  const toolInspector = await evaluate("document.querySelector('[data-inspector-context=\"tool\"]')?.textContent");
  await clickSelector('[data-testid="tool-select"]');
  await waitFor("document.querySelector('[data-inspector-context=\"slide\"]')");
  const slideInspector = await evaluate("document.querySelector('[data-inspector-context=\"slide\"]')?.textContent");
  await send("selection", { target: secondShape, mode: "replace" });
  await waitFor("document.querySelector('.inspector .node-summary')");
  const objectInspector = await evaluate("document.querySelector('.inspector')?.textContent");
  await capture(phase1bScreenshots.timeline);

  const activeDevice = gpuInfo.gpu?.devices?.find((device) => device.active) ?? gpuInfo.gpu?.devices?.[0] ?? {};
  const beforeRevisions = new Map(beforeEdit.editor_session.slides.map((slide) => [slide.id, slide.thumbnail_revision]));
  const changedSlides = afterEdit.editor_session.slides.filter((slide) => slide.thumbnail_revision !== beforeRevisions.get(slide.id));
  const checks = {
    server_health: preview.health.ok === true,
    fresh_chrome_process: Boolean(chromeProcess.pid),
    fresh_temporary_profile: profile.includes("phase1b-chrome-"),
    actual_webgpu: initial.actual_webgpu === true,
    dedicated_worker_wasm: initial.worker_runtime_owner === "dedicated-worker" && initial.wasm_initialized === true,
    hardware_device: Boolean(activeDevice.deviceString || activeDevice.deviceId),
    full_hd_default_slide: initial.editor_session.slides[0].width === 1920 && initial.editor_session.slides[0].height === 1080,
    slide_add_and_active_state: afterAdd.editor_session.slides.length === 2 && secondSlide !== firstSlide,
    per_slide_selection_camera_restore:
      restoredFirst.selection.ordered.includes(firstShape) &&
      JSON.stringify(restoredFirst.camera) === JSON.stringify(firstState.camera) &&
      restoredSecond.selection.ordered.includes(secondShape) &&
      JSON.stringify(restoredSecond.camera) === JSON.stringify(secondState.camera),
    active_slide_hit_isolation:
      firstHit.result.topmost === firstShape && !firstHit.result.all.includes(secondShape) &&
      secondHit.result.topmost === secondShape && !secondHit.result.all.includes(firstShape),
    other_slide_selection_blocked: outsideSelection?.code === "node_outside_active_slide",
    deep_duplicate_independent_ids: duplicateChildren.length === 1 && duplicateChildren[0] !== secondShape,
    rename_reorder_persisted:
      reordered.editor_session.slides[0].id === duplicateSlide &&
      reordered.editor_session.slides[0].name === "Opening copy" &&
      restored.editor_session.slides[0].id === duplicateSlide &&
      restored.editor_session.slides[0].name === "Opening copy",
    delete_undo_redo_global_history:
      deleted.editor_session.slides.length === 2 &&
      undone.editor_session.slides.some((slide) => slide.id === duplicateSlide) &&
      !redone.editor_session.slides.some((slide) => slide.id === duplicateSlide),
    isolated_thumbnail_invalidation: changedSlides.length === 1 && changedSlides[0].id === secondSlide,
    bounded_thumbnail_queue:
      afterEdit.thumbnails.max_renders_per_engine_frame === 1 &&
      afterEdit.thumbnails.priority_policy === "active-visible-remaining" &&
      afterEdit.thumbnails.render_source === "rust-render-model-slots" &&
      afterEdit.thumbnails.queue_depth === 0,
    actual_webgpu_thumbnails: afterEdit.thumbnails.cached === 3 && afterEdit.thumbnails.canvas2d_fallback_count === 0,
    three_accessible_splitters: panelState.splitter_count === 3,
    splitter_preferences_not_document:
      panelState.after.slidesWidth !== panelState.before.slidesWidth &&
      afterEdit.revisions.document === (await getProof()).revisions.document,
    layers_timeline_row_sync:
      panelState.layer_ids.length > 0 && JSON.stringify(panelState.layer_ids) === JSON.stringify(panelState.timeline_ids),
    contextual_inspector:
      toolInspector.includes("Ellipse Tool") && slideInspector.includes("Slide properties") && objectInspector.includes("Appearance"),
    worker_gpu_overlay_sequence_match: afterEdit.response_gpu_overlay_sequence_match === true,
    no_console_errors: consoleErrors.length === 0,
    no_gpu_validation_errors: afterEdit.gpu.validation_errors === 0,
    no_fallback_rebuilds: maxFallbackRebuildCountSeen === 0,
  };

  const screenshotInfo = {};
  for (const [name, pathname] of Object.entries(phase1bScreenshots)) {
    const details = await stat(pathname);
    screenshotInfo[name] = { path: path.relative(workspace, pathname).replaceAll("\\", "/"), bytes: details.size };
  }
  const proof = {
    phase: "1B",
    proof_kind: "actual-hardware-browser",
    ...
    chrome_process_id: chromeProcess.pid,
    chrome_profile: profile,
    browser_mode: "fresh Chrome; actual Dedicated Worker + WASM EngineHost + WebGPU; no mock; no Canvas2D fallback",
    execution: {
    ...
    initial,
    final: afterEdit,
    interactions: { firstCreated, secondCreated, firstState, secondState, restoredFirst, firstHit, outsideSelection, restoredSecond, secondHit, duplicated, reordered, deleted, undone, redone, restored, panelState },
    screenshots: screenshotInfo,
    console_errors: consoleErrors,
    chrome_stderr_tail: chromeStderr,
    max_fallback_rebuild_count_seen: maxFallbackRebuildCountSeen,
  ...
  await writeFile(proofPath, JSON.stringify(proof, null, 2) + "\n", "utf8");
  console.log("Phase 1B targeted actual browser proof: " + (proof.all_passed ? "PASS" : "FAIL"));
  console.log("browser=" + version.Browser);
  console.log("gpu=" + (activeDevice.deviceString ?? activeDevice.deviceId ?? "unknown"));
  console.log("proof=" + proofPath);
  if (!proof.all_passed) {
  ...
  const browserDiagnostics = pageClient
    ? await evaluate("({body_ready: document.body?.dataset.ready ?? null,state: window.__phase0eState ?? null,proof: window.__PHASE0E_PROOF__ ?? null,navigator_gpu: Boolean(navigator.gpu)})").catch((diagnosticError) => ({ diagnostic_error: diagnosticError.message }))
    : null;
  const failure = {
    phase: "1B",
    proof_kind: "actual-hardware-browser",
